...
//     // Create a URL pointing to your inventory page with the item ID
//     const inventoryItemUrl = `${window.location.origin}/inventory/${item.id}`;
...
function App() {
  ...
  useEffect(() => {
    ...
    const autoQRPref = localStorage.getItem('autoGenerateQR');
    if (autoQRPref !== null) setAutoGenerateQR(JSON.parse(autoQRPref));

    const storedSpace = localStorage.getItem('totalWarehouseSpace');
    if (storedSpace !== null) setTotalWarehouseSpace(parseFloat(storedSpace));

    const storedColors = localStorage.getItem('categoryColors');
    if (storedColors !== null) setCategoryColors(JSON.parse(storedColors));
  }, []);

  // Save data to localStorage
  useEffect(() => localStorage.setItem('inventory', JSON.stringify(inventory)), [inventory]);
  useEffect(() => localStorage.setItem('autoGenerateQR', JSON.stringify(autoGenerateQR)), [autoGenerateQR]);
  useEffect(() => localStorage.setItem('totalWarehouseSpace', totalWarehouseSpace.toString()), [totalWarehouseSpace]);
  ...
  const calculateTotalSpaceUsed = () => inventory.reduce((total, item) => total + ((item.spacePerUnit || 0) * item.quantity), 0);
  const getSpaceUsedPercentage = () => (calculateTotalSpaceUsed() / totalWarehouseSpace) * 100;
  ...
  const handleSubmit = (e) => {
    ...
    if (currentMode === 'create') {
      ...
      item.id = formData.id;
      item.dateAdded = formData.dateAdded;
  ...
  const generateQRCodeUrl = (item) => {
    const inventoryItemUrl = `${window.location.origin}/#/inventory/${item.id}`;
    return `https://api.qrserver.com/v1/create-qr-code/?data=${encodeURIComponent(inventoryItemUrl)}&size=300x300`;
  };
    ...
    html5QrcodeScanner.render((decodedText) => {
      try {
        if (decodedText.includes('/inventory/')) {
          const itemId = decodedText.split('/').pop();
        ...
        if (existingItem) {
          ...
          showAlert('Item not found in inventory');
        }
      } catch (error) {
        console.error('Invalid QR data:', error);
        showAlert('Invalid QR code format');
  ...
  const chartData = inventory.length > 0 ? inventory
    .sort((a, b) => b.quantity - a.quantity)
    .slice(0, 5)
    .map(item => ({
      name: item.name,
  ...
  const updateWarehouseSpace = (newSpace) => setTotalWarehouseSpace(parseFloat(newSpace));
  ...
  return (
    <div className="app-container">
      ...
      <div className="tabs">
        {['dashboard', 'inventory', 'form', 'scanner', 'settings'].map(tab => (
          <button 
            key={tab}
          ...
          >
            <span className="material-icons">
              {tab === 'dashboard' ? 'dashboard' :
               ...
               tab === 'form' ? 'add_circle' :
               tab === 'scanner' ? 'qr_code_scanner' : 'settings'}
            </span>
            {tab === 'form' ? (currentMode === 'create' ? 'Add Item' : 'Edit Item') : tab.charAt(0).toUpperCase() + tab.slice(1)}
          </button>
        ))}
      </div>
...
function InventoryItemDetail({ inventory, onEdit, onDelete, onShowQR, getCategoryColor }) {
  const { id } = useParams();
  const navigate = useNavigate();
      ...
      <div className="detail-content">
        ...
        <div className="detail-section">
          ...
          <div className="detail-row">
            ...
            <span className={`detail-value ${item.quantity < 5 ? 'low-stock' : ''}`}>
              {item.quantity}
            </span>
          </div>
          <div className="detail-row">
          ...
          <div className="detail-row">
            ...
            <span className="detail-value">₹{(item.price * item.quantity).toFixed(2)}</span>
          </div>
          {item.spacePerUnit > 0 && (
            <div className="detail-row">
              <span className="detail-label">Space Used:</span>
              <span className="detail-value">
                {(item.spacePerUnit * item.quantity).toFixed(2)} m³
              </span>
            </div>
          )}
        </div>
        ...
        <div className="detail-section">
          ...
          <div className="detail-row">
            <span className="detail-label">Brought By:</span>
            <span className="detail-value">{item.broughtBy || 'Not specified'}</span>
          </div>
          <div className="detail-row">
