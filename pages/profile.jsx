...
const Profile = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Clear any pending updates from local storage
      localStorage.removeItem('pendingProfileUpdate');
      setHasPendingUpdate(false);
      setEditMode(false);
    } catch (err) {
  ...
  // Function to change photo only
  const changePhotoOnly = async () => {
    ...
    try {
      const photoURL = await uploadImage(profile.profileImage);
      
      // Update Firebase Auth photoURL
      await updateProfile(auth.currentUser, { photoURL });
      
      // Update local user context with new photoURL
      updateUserData({
        ...userData,
        photoURL
      });
    ...
    <div className="profile-container">
      ...
      <div className="profile-content">
        <div className="profile-image-section">
          ...
          {/* Add a button to update photo only */}
          {editMode && profile.profileImage instanceof File && (
            <button 
              className="change-photo-btn"
              onClick={changePhotoOnly}
              disabled={isSaving || isUploading}
            >
              {isUploading ? 'Uploading...' : 'Update Photo Only'}
            </button>
          )}
        </div>
      ...
      {editMode && (
        <div className="action-buttons">
          <button className="cancel-btn" onClick={handleCancel} disabled={isSaving || isUploading}>
            Cancel
          </button>
